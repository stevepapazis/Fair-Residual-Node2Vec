...
void graph::add_edge(const int src_node, const int dest_node)
{
	...
	nodes[src_node].out_neighbor_ids.insert(nodes[dest_node].id);
...
void graph::set_residual_types(double phi)
{
	for(int i=0; i<nnodes; i++)
	{
		if(count_out_neighbors_with_community(i, 1)/get_out_degree(i) < phi)
...
void graph::calc_residuals(double phi)
{
	residuals = new double[nnodes];
	for(int i=0; i<nnodes; i++)
	{
		if(nodes[i].residual_type == 1)
			if(count_out_neighbors_with_community(i, 0) != 0)
				residuals[i] = phi - (((1 - phi) * count_out_neighbors_with_community(i, 1)) / (count_out_neighbors_with_community(i, 0)));
			else
				residuals[i] = phi;
		else
			if(count_out_neighbors_with_community(i, 1) != 0)
				residuals[i] = (1 - phi) - ((phi * count_out_neighbors_with_community(i, 0)) / count_out_neighbors_with_community(i, 1));
			else
				residuals[i] = 1 - phi;
	}
